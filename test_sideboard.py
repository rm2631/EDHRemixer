...
def test_sideboard_default():
    ...
    assert (
        c.include_sideboard == False
    ), f"Expected include_sideboard False, got {c.include_sideboard}"
...
def test_sideboard_explicit_true():
    ...
    assert (
        c.include_sideboard == True
    ), f"Expected include_sideboard True, got {c.include_sideboard}"
...
def test_filter_sideboard_collections():
    ...
    collections = [
        ...
        Collection(
            name="Default", url="https://www.moxfield.com/decks/c", is_source=False
        ),
    ]
    ...
    assert (
        len(with_sideboard) == 1
    ), f"Expected 1 collection with sideboard, got {len(with_sideboard)}"
