...
def test_filter_inactive_collections():
    ...
    assert (
        len(active_collections) == 2
    ), f"Expected 2 active collections, got {len(active_collections)}"
    assert all(
        c.active for c in active_collections
    ), "All filtered collections should be active"
    assert "Inactive" not in [
        c.name for c in active_collections
    ], "Inactive collection should be filtered out"
